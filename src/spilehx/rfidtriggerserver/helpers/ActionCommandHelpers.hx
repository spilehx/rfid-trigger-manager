...
import spilehx.rfidtriggerserver.managers.actionmanager.ProcessWrapper;
...
class ActionCommandHelpers {
	...
	public static var MOPIDY_NONDOCKER_CONFIG_PATH:String = "./appdata/mopidy/mopidy.conf";
	...
	public static function startMopidy(onComplete:Bool->Void = null):Void {
		try {
			...
			var mopidyCommand:String = "nohup mopidy --config " + MOPIDY_NONDOCKER_CONFIG_PATH + " >/dev/null 2>&1 &";
	...
	private static function ensureMopidyConfig() {
		...
		if (FileSystem.exists(MOPIDY_CONFIG_PATH) == false) {
			...
			FileSystemHelpers.ensurePath(MOPIDY_CONFIG_FOLDER_PATH);
...
class YTActionCommandHelpers {
	public static function getYTPlayListTracks(playlistId:String, onLoaded:Array<String>->Void) {
		USER_MESSAGE("Getting YT playlist tracks:" + playlistId);

		ProcessWrapper.instance.runProcessForResponse("yt-dlp", ["--flat-playlist", "--get-id", playlistId], function(resp:Dynamic) {
			...
			if (validResponse) {
				...
				USER_MESSAGE("Bad playlist, no tracks found for "+playlistId);
				onLoaded(new Array<String>());
			}
		});
	}
}